...
on the command line (or a set) and runs GreedyBestPuzzleSolver on it.
"""
...
for i in range(1 if (puzz_set is None) else 0, num_puzzles):
    ...
    # run the solver 
    start = timer()
    solver = GreedyBestPuzzleSolver(initial_board = board, graph_search = GRAPH_SEARCH, heuristic_fn = HEURISTIC_FN)  
