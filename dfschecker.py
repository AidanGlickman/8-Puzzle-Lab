...
on the command line (or a set) and runs DFSPuzzleSolver on it.
"""
...
for i in range(1 if (puzz_set is None) else 0, num_puzzles):
    ...
    if (puzz_set is not None) and AUTO_MAX_DEPTH :
        solver = DFSPuzzleSolver(initial_board = board, graph_search = GRAPH_SEARCH, max_depth = i) 
    else:
        solver = DFSPuzzleSolver(initial_board = board, graph_search = GRAPH_SEARCH, max_depth = MAX_DEPTH) 
    end = timer()
